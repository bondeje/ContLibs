...
Slice * Slice_new(size_t start, size_t end, long long step) {
	// malloc a new slice
	Slice * sl = NULL;
	if (step != 0 && (end - start) / step > 0 && (sl = (Slice *) CL_MALLOC(sizeof(Slice)))) {	
...
size_t SliceIterator_next(SliceIterator * sl_iter) {
	...
	} else if (sl_iter->stop == ITERATOR_GO) {
		if ((sl_iter->sl.step < 0 && (sl_iter->next - sl_iter->sl.end < -sl_iter->sl.step)) || (sl_iter->sl.step > 0 && (sl_iter->sl.end - sl_iter->next < sl_iter->sl.step))) {
			sl_iter->stop = ITERATOR_STOP;
		} else {
...
void SliceIteratorIterator_init(SliceIteratorIterator * sl_iter_iter, SliceIterator * sl_iter) {
	if (!sl_iter_iter) {
        return;
    }
    if (!sl_iter) {
        sl_iter_iter->stop = ITERATOR_STOP;
        return;
    }
    *sl_iter_iter = *sl_iter;   
}

size_t SliceIteratorIterator_next(SliceIteratorIterator * sl_iter) {
	return SliceIterator_next(sl_iter);
}

enum iterator_status SliceIteratorIterator_stop(SliceIteratorIterator * sl_iter) {
